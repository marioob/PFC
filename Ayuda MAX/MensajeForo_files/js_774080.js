...
function lm(tid, el, mid, bId, messId, lt, align, showImmediate)
{
	...
	if (tid == openMenuId) {
		...
		getDocument("lazymenu").location.href="/ni/lazymenu?"+queryString+"";
	} else {
		ddId = "";
		curEl = "";
		mAlign = "";
...
function note_frame() {
	if (!noteswin.closed)
		noteswin.focus();
	noteswin=window.open("/ni/notes?action=frame", "notes" + user_id() + "ni", "width=600,height=400,scrollbars=yes,resizable=yes");
}
function friends()    {
	if (!noteswin.closed)
		noteswin.focus();
	noteswin=window.open("/ni/notes?action=frame&tab=4", "notes" + user_id() + "ni", "width=600,height=400,scrollbars=yes,resizable=yes");
}
function note_send(to_user) {
	if (!noteswin.closed)
		noteswin.focus();
	noteswin=window.open("/ni/notes?action=frame&tab=3&toUser="+to_user+"", "notes" + user_id() + "ni", "width=600,height=400,scrollbars=yes,resizable=yes");
}		
function note_view(id) {
	if (!noteswin.closed)
		noteswin.focus();
	noteswin=window.open("/ni/notes?action=frame&tab=7&t=inbox&id="+id+"", "notes" + user_id() + "ni", "width=600,height=400,scrollbars=yes,resizable=yes");
}		
function assistant()    {
	if (!assistantwin.closed)
		assistantwin.focus();
	assistantwin=window.open("/ni/assistant", "assistant" + user_id() + "ni", "width=300,height=400,scrollbars=yes,resizable=yes");
}
...
function moveBatch(field) {
	if (field) {
		...
		if (checked) 
			lipopup("/ni/board/move_message?mp="+mp+"", 'move', 600, 635, 'yes', 'yes')
		else
